...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await api.auth.register(formData.email, formData.password, formData.name);
  ...
  const getPasswordStrength = (password: string) => {
    if (password.length === 0) return { color: 'text-gray-400', text: 'Enter a password' };
    if (password.length < 6) return { color: 'text-red-500', text: 'Weak' };
    if (password.length < 8) return { color: 'text-yellow-500', text: 'Fair' };
    if (password.match(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/)) return { color: 'text-green-500', text: 'Strong' };
    return { color: 'text-blue-500', text: 'Good' };
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-brand-50 to-pink-50 py-12 px-4 sm:px-6 lg:px-8">
      ...
      >
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Join MYGlamBeauty for exclusive offers and easy booking
          </p>
        </div>
